...
export default function Home() {
  ...
  const [currentReviewIndex, setCurrentReviewIndex] = useState(0);
  const [isMobile, setIsMobile] = useState(false);
  // Track hover status to pause rotation
  ...
  // 3. Auto Rotate Carousel (Pauses on Hover)
  useEffect(() => {
    if (isHovered) return;
                ...
                {REVIEWS.map((review, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      onClick={() => setCurrentReviewIndex(index)}
                      // ONLY pause rotation if hovering over the active center card
                      onMouseEnter={() => {
           ...
           <div className="flex justify-center gap-2 mt-10">
             {REVIEWS.map((_, idx) => (
               <button 
                 key={idx} 
                 onClick={() => setCurrentReviewIndex(idx)}
                 className={`h-2 rounded-full transition-all duration-300 cursor-pointer ${idx === currentReviewIndex ? "w-8 bg-[#D98292]" : "w-2 bg-[#D98292]/30"}`}
               />
             ))}
           </div>
