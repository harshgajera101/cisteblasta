...
//       setToast({
//         show: true,
//         message: "Please fix contact info errors.",
//         type: "error",
//       });
//       return;
//     }
...
//       setToast({
//         show: true,
//         message: "Please detect your location for delivery.",
//         type: "error",
//       });
//       return;
//     }
...
//               </div>
//               <h2 className="text-3xl font-playfair font-bold text-[#4E342E] mb-2">
//                 Order Placed!
//               </h2>
//               <p className="text-[#8D6E63] mb-8">
//                 Your order details have been sent to WhatsApp. Please complete
//                 the conversation there to finalize payment.
//               </p>
//               <Link href="/menu">
...
//               <span className="text-[#D98292] lowercase italic font-normal">
//                 (optional)
//               </span>
//             </label>
//             <textarea
...
//               Any message or customization request for this order.
//             </p>
//           </div>

//           {/* NEW: Important Note / Disclaimer Card (Matched Style) */}
//           <div className="bg-[#FFF8F3] p-6 rounded-2xl border border-[#F2E3DB] space-y-3">
//             <h4 className="text-sm font-bold text-[#4E342E] flex items-center gap-2 mb-2">
//               <AlertTriangle size={16} className="text-[#D98292]" /> Important
//               Note
//             </h4>
//             <ul className="space-y-2">
//               <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed">
//                 <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" />
//                 <span>
//                   Only advance payment orders are accepted (Full advance payment
//                   for half kg cakes and 50% advance for 1kg).
//                 </span>
//               </li>
//               <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed">
//                 <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" />
//                 <span>
//                   Please place your order at least 2-3 days in advance.
//                 </span>
//               </li>
//               <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed">
//                 <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" />
//                 <span>
//                   Delivery charges are calculated based on distance and will be
...
//                 <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" />
//                 <span>
//                   For customized cake you can check our Instagram page{" "}
//                   <a
//                     href="https://www.instagram.com/blastaciste"
//                     target="_blank"
...
//                 <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" />
//                 <span>
//                   After clicking "Order Now", you will be redirected to WhatsApp
//                   to confirm payment.
//                 </span>
//               </li>
//               <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed">
//                 <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" />
//                 <span>
//                   Orders are only confirmed once the payment screenshot is
//                   shared.
//                 </span>
//               </li>
//             </ul>
...
//                     className="text-xs font-bold text-[#D98292] flex items-center gap-1 hover:underline"
//                   >
//                     {isEditingContact ? (
//                       <>
//                         <Check size={12} /> Done
...
//                     <Phone
//                       className="absolute left-3 top-3 text-[#D98292]"
//                       size={16}
//                     />
//                     <input
//                       type="tel"
...
//                           : isEditingContact
//                           ? "bg-white border-[#D98292] text-[#4E342E]"
//                           : "bg-[#FFF8F3] border-[#F2E3DB] text-[#8D6E63] cursor-not-allowed"
//                       }`}
//                     />
//                   </div>
//                   {contactErrors.phone && (
//                     <p className="text-xs text-red-500 ml-1 flex items-center gap-1">
//                       <AlertCircle size={10} /> {contactErrors.phone}
//                     </p>
//                   )}
//                 </div>
...
//                 <LogIn size={24} />
//               </div>
//               <h3 className="font-playfair font-bold text-xl text-[#4E342E] mb-2">
//                 Login to Checkout
//               </h3>
//               <p className="text-sm text-[#8D6E63] mb-6">
//                 Please sign in to place your order. This helps us track your
//                 delivery.
//               </p>
//               <Link
//                 href="/login?callbackUrl=/cart"
//                 className="block w-full py-3 bg-[#4E342E] text-white font-bold rounded-xl shadow hover:bg-[#3d2924] transition-all"
//               >
//                 Login / Signup
//               </Link>
//             </div>
...
//               <Link
//                 href="/login?callbackUrl=/cart"
//                 className="block w-full py-4 mt-2 bg-[#D98292] text-white font-bold rounded-xl shadow-lg hover:bg-[#c46b7d] text-center"
//               >
//                 Login to Order
//               </Link>
//             )}
...
import { motion, AnimatePresence } from "framer-motion";
import { useSession } from "next-auth/react";
import { Trash2, Plus, Minus, MapPin, Navigation, ArrowRight, Phone, Mail, Edit2, Check, LogIn, AlertCircle, Info, AlertTriangle, CheckCircle2 } from "lucide-react";
import { Toast } from "@/components/ui/Toast";
import Link from "next/link";
import { calculateDistance, KITCHEN_COORDS, DELIVERY_RATE_PER_KM, MIN_DELIVERY_CHARGE } from "@/lib/utils";
import { DELIVERY_PINCODES } from "@/lib/pincodes"; 
...
export default function CartPage() {
  ...
  useEffect(() => {
    if (session?.user) {
      setContactInfo({ phone: (session.user as any).phone || "", email: session.user.email || "" });
      fetchSavedAddress();
    }
  }, [session]);
  ...
  const fetchSavedAddress = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Failed");
      const data = await res.json();
      if (data.success && data.user.address) setAddress(data.user.address);
    } catch (e) {}
  };

  const deliveryCharge = deliveryDistance ? Math.max(Math.round(deliveryDistance * DELIVERY_RATE_PER_KM), MIN_DELIVERY_CHARGE) : 0;
  const grandTotal = cartTotal + deliveryCharge;

  // Pincode Verification
  const verifyPincode = () => {
    if (!pincode.trim() || pincode.length !== 6) {
      setPincodeMsg({ text: "Please enter a valid 6-digit PIN code.", isError: true });
      setIsPincodeVerified(false);
      return;
    }
    const area = DELIVERY_PINCODES[pincode];
    if (area) {
      setPincodeMsg({ text: `Delivery available in ${area}! 🚚`, isError: false });
      setIsPincodeVerified(true);
    } else {
      setPincodeMsg({ text: "Coming soon to your area! Currently delivering only between Virar & Santacruz.", isError: true });
      setIsPincodeVerified(false);
      setDeliveryDistance(null); // Reset location if they were previously verified but changed it
    }
  };
  ...
  const validateContact = () => {
    ...
    if (!contactInfo.email || !/^[a-zA-Z0-9._%+-]+@gmail\.com$/.test(contactInfo.email)) {
      errors.email = "Please enter a valid @gmail.com address";
      isValid = false;
    }
    setContactErrors(errors);
  ...
  const handleOrder = async () => {
    ...
    if (!isPincodeVerified) {
      setToast({ show: true, message: "Please verify your delivery PIN code first.", type: "error" });
      return;
    }
    if (!validateContact()) {
      setIsEditingContact(true);
    ...
    try {
      const res = await fetch("/api/order/checkout", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          customerDetails: { address, phone: contactInfo.phone, email: contactInfo.email, name: customerName },
          items, instructions,
          bill: { itemTotal: cartTotal, deliveryCharge, distance: deliveryDistance, grandTotal },
        }),
      });
      ...
      if (data.success) {
        ...
        message += `👤 *Customer:* ${customerName}\n`;
        message += `📞 *Phone:* ${contactInfo.phone}\n`;
        message += `📍 *Delivery Location:* ${address} (PIN: ${pincode})\n`; 
        message += `----------------------------\n`;
        items.forEach((item) => { message += `▫️ ${item.quantity} x ${item.name} (${item.variant || "Std"}) - ₹${item.price * item.quantity}\n`; });
        message += `----------------------------\n`;
        if (instructions) message += `📝 *Note:* ${instructions}\n`;
  ...
  if (items.length === 0 && !showSuccessModal) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 text-center">
        <div className="w-24 h-24 bg-[#FFF8F3] rounded-full flex items-center justify-center mb-6"><MapPin size={40} className="text-[#D98292] opacity-50" /></div>
      ...
      <AnimatePresence>
        {showSuccessModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 text-center">
              ...
              <p className="text-[#8D6E63] mb-8">Your order details have been sent to WhatsApp. Please complete the conversation there to finalize payment.</p>
              <Link href="/menu"><button className="w-full py-4 bg-[#4E342E] text-white font-bold rounded-xl shadow-lg hover:bg-[#3d2924] transition-all">Continue Shopping</button></Link>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            <AnimatePresence>
              {items.map((item) => (
                <motion.div key={`${item.productId}-${item.variant}`} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -100 }} className="flex gap-4 md:gap-6 p-4 bg-white rounded-2xl shadow-sm border border-[#F2E3DB]">
                  <div className="h-24 w-24 shrink-0 rounded-xl bg-[#FFF8F3] flex items-center justify-center overflow-hidden border border-[#F2E3DB]">{item.image ? (<img src={item.image} alt={item.name} className="h-full w-full object-cover" />) : (<span className="text-[#D98292]/30 font-playfair font-bold text-2xl">{item.name.charAt(0)}</span>)}</div>
                  <div className="flex-1 flex flex-col justify-between"><div><h3 className="font-bold text-[#4E342E] text-lg leading-tight">{item.name}</h3><p className="text-sm text-[#8D6E63] mt-1">{item.variant ? item.variant : "Standard"}{item.category === "GIFT_BOX" && " • Box"}</p></div><div className="flex items-center justify-between mt-3"><div className="font-bold text-[#4E342E]">₹{item.price * item.quantity}</div><div className="flex items-center gap-3 bg-[#FFF8F3] rounded-lg p-1"><button onClick={() => item.quantity === 1 ? removeFromCart(item.productId, item.variant) : updateQuantity(item.productId, item.variant, -1)} className="p-1 hover:bg-white rounded-md transition-colors text-[#4E342E]">{item.quantity === 1 ? (<Trash2 size={14} className="text-red-500" />) : (<Minus size={14} />)}</button><span className="text-sm font-bold w-4 text-center">{item.quantity}</span><button onClick={() => updateQuantity(item.productId, item.variant, 1)} className="p-1 hover:bg-white rounded-md transition-colors text-[#4E342E]"><Plus size={14} /></button></div></div></div>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#F2E3DB] space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold uppercase tracking-wider text-[#8D6E63] flex items-center gap-2">
                <MapPin size={16} className="text-[#D98292]" /> Check Availability <span className="text-red-500">*</span>
              </label>
              {isPincodeVerified && <CheckCircle2 size={16} className="text-green-600" />}
            </div>
            <div className="flex gap-2">
              <input 
                ...
                onChange={(e) => {
                  ...
                  setIsPincodeVerified(false);
                  setPincodeMsg({ text: "", isError: false });
                }}
                className={`flex-1 rounded-lg border p-3 text-sm focus:outline-none transition-colors ${isPincodeVerified ? 'border-green-300 bg-green-50 text-[#4E342E]' : 'border-[#F2E3DB] bg-[#FFF8F3]/50 focus:border-[#D98292] text-[#4E342E]'}`}
              />
              <button 
                onClick={verifyPincode}
                className={`px-6 rounded-lg font-bold text-sm transition-colors ${isPincodeVerified ? 'bg-green-600 text-white' : 'bg-[#D98292] text-white hover:bg-[#c46b7d]'}`}
              >
                Verify
              </button>
            </div>
            {pincodeMsg.text && (
              <p className={`text-xs font-bold flex items-start gap-1 mt-1 ${pincodeMsg.isError ? 'text-red-500' : 'text-green-600'}`}>
                {pincodeMsg.isError ? <AlertTriangle size={12} className="shrink-0 mt-0.5" /> : null}
                {pincodeMsg.text}
              </p>
            )}
          </div>
          ...
          {/* Special Instructions Section */}
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#F2E3DB] space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-[#8D6E63] flex items-center gap-2"><Info size={14} /> Special Instructions <span className="text-[#D98292] lowercase italic font-normal">(optional)</span></label>
            <textarea rows={2} value={instructions} onChange={(e) => setInstructions(e.target.value)} placeholder="e.g. Write 'Happy Birthday' or 'Less Sugar'..." className="w-full rounded-lg border border-[#F2E3DB] bg-[#FFF8F3]/50 p-3 text-[#4E342E] focus:outline-none focus:ring-2 focus:ring-[#D98292]/50 resize-none"></textarea>
            <p className="text-[10px] text-[#8D6E63]">Any message or customization request for this order.</p>
          </div>

          {/* Important Note */}
          <div className="bg-[#FFF8F3] p-6 rounded-2xl border border-[#F2E3DB] space-y-3">
            <h4 className="text-sm font-bold text-[#4E342E] flex items-center gap-2 mb-2"><AlertTriangle size={16} className="text-[#D98292]" /> Important Note</h4>
            <ul className="space-y-2">
              <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" /><span>Only advance payment orders are accepted (Full advance payment for half kg cakes and 50% advance for 1kg).</span></li>
              <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" /><span>Please place your order at least 2-3 days in advance.</span></li>
              <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" /><span>Delivery charges are calculated based on distance and will be added to the final bill.</span></li>
              <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" /><span>For customized cake you can check our Instagram page <a href="https://www.instagram.com/blastaciste" target="_blank" rel="noopener noreferrer" className="underline cursor-pointer">@blastaciste</a> and <a href="/custom-cake" target="_blank" rel="noopener noreferrer" className="underline cursor-pointer">gallery</a> present in the website.</span></li>
              <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" /><span>After clicking "Order Now", you will be redirected to WhatsApp to confirm payment.</span></li>
              <li className="flex items-start gap-3 text-xs text-[#8D6E63] leading-relaxed"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#D98292] shrink-0" /><span>Orders are only confirmed once the payment screenshot is shared.</span></li>
            </ul>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {session ? (
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#F2E3DB] space-y-4">
              ...
              <div className="space-y-3 pb-4 border-b border-[#F2E3DB]">
                <div className="flex justify-between items-center"><label className="text-xs font-bold uppercase tracking-wider text-[#8D6E63]">Contact Info <span className="text-red-500">*</span></label><button onClick={() => setIsEditingContact(!isEditingContact)} className="text-xs font-bold text-[#D98292] flex items-center gap-1 hover:underline">{isEditingContact ? (<><Check size={12} /> Done</>) : (<><Edit2 size={12} /> Change</>)}</button></div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-[#8D6E63]">Location <span className="text-red-500">*</span></label>
                {!deliveryDistance ? (
                  <button onClick={handleGetLocation} disabled={isLocating || !isPincodeVerified} className="w-full py-3 border-2 border-dashed border-[#D98292] text-[#D98292] font-bold rounded-xl hover:bg-[#D98292]/5 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                    {isLocating ? (<span className="animate-pulse">Locating...</span>) : (<> <Navigation size={18} /> Detect My Location </>)}
                  </button>
                ) : (
                  <div className="flex items-center justify-between bg-[#effaf0] border border-green-200 p-3 rounded-xl"><span className="text-green-700 text-sm font-bold flex items-center gap-2"><MapPin size={16} /> {deliveryDistance.toFixed(1)} km away</span><button onClick={() => setDeliveryDistance(null)} className="text-xs text-green-700 underline">Change</button></div>
                )}
                {locError && <p className="text-xs text-red-500">{locError}</p>}
                {!isPincodeVerified && <p className="text-[10px] text-red-500 font-bold">* Please verify PIN Code first</p>}
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-[#8D6E63]">Full Address <span className="text-red-500">*</span></label>
          ...
          <div className="bg-[#FFF8F3] p-6 rounded-2xl border border-[#F2E3DB] space-y-4">
            ...
            {session ? (
              <button 
                onClick={handleOrder} 
                disabled={isOrdering || !isPincodeVerified} 
                className="w-full py-4 mt-2 bg-[#D98292] text-white font-bold rounded-xl shadow-lg hover:bg-[#c46b7d] hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
